...
@push('script')
        ...
        $(document).ready(function() {
            let dTable = $('#basic_tables').DataTable({
                ...
                columns: [{
                    ...
                    let preview = words.slice(0, 100).join(' ') + (words.length > 100 ? '... <a href="#" class="show-more">Show More</a>' : '');
    ...
    $('#basic_tables').on('click', '.show-more', function(e) {
        e.preventDefault();
        const fullContent = $(this).closest('td').data('full-content');  // Store full content in a data attribute
        
        // Replace the preview with full content
        $(this).closest('td').html(fullContent);
    });

    // Custom search and pagination code remains unchanged
    $('#customSearchBox').on('keyup', function() {
        dTable.search(this.value).draw();
    });
